...
- Product catalog (26 SKUs across phones/tablets/computers/gaming/accessories, incl. the current Apple and Samsung lineups added 2026-09-06 at the client's request) and repair price list (7 services) exist and should be preserved as real demo data. The 12 newest SKUs (iPhone 17 line, MacBook Pro, iPad Pro/base, Apple Watch Ultra 2, AirPods Pro 2/4, Galaxy S25/S25 Ultra, Galaxy Watch7) don't have local photos yet and render via the icon fallback — real photos should be sourced and dropped into `img/` before this goes live.
...
- Product imagery: every catalog SKU now has a real product photo/render, sourced from Wikimedia Commons (freely licensed) and mirrored locally in `img/` so the demo is fully self-contained (no hotlink rate limits, works offline). Exact Commons source file for each image is documented in a comment above the `IMG` map in `app.js`. The authored line icons remain as the runtime fallback via `pmedia()`'s `onerror` path. Do not fabricate customer-facing claims beyond the verified facts above.
