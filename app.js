...
/* ---------- brand strip ----------
   ...
   loops seamlessly while one copy is at least as wide as the viewport — at 1900px
   each copy was 1176px, so the tail ran out and left visible empty space. It also
   inherited direction:rtl, which flipped the travel direction against the layout.
   This builds the strip from BRAND_MARQUEE, clones the group until the track is at
   least twice the container width, and shifts by exactly one group width in px, so
   it is endless at any width. Speed is held constant (px/sec) rather than a fixed
   duration, otherwise a wider group would scroll faster. */
const MARQUEE_SPEED=52; /* px per second */
function renderBrandMarquee(){
  const wrap=document.querySelector('.brand-marquee'); if(!wrap) return;
  const track=wrap.querySelector('.marquee-track'); if(!track) return;
  const group=BRAND_MARQUEE.map(b=>
    `<span class="marquee-logo"><svg viewBox="${b.vb}" style="--h:${b.h}px" fill="currentColor" role="img" aria-label="${b.n}"><path d="${b.d}"/></svg></span>`
  ).join('');
  track.innerHTML=`<div class="marquee-group">${group}</div>`;
  const first=track.firstElementChild;
  const groupW=first.getBoundingClientRect().width;
  if(!groupW){ return; }                       /* not laid out yet (hidden page) */
  const containerW=wrap.getBoundingClientRect().width||window.innerWidth;
  /* need track >= container + one whole group, so after shifting by a group there is
     still a full container's worth of content ahead; +2 keeps a spare group in hand */
  const copies=Math.max(2, Math.ceil(containerW/groupW)+2);
  for(let i=1;i<copies;i++) track.appendChild(first.cloneNode(true));
  track.style.setProperty('--marquee-shift', groupW+'px');
  track.style.setProperty('--marquee-duration', (groupW/MARQUEE_SPEED).toFixed(2)+'s');
}
...
function initBrandMarquee(){
  ...
  addEventListener('resize',()=>{
    ...
    marqueeResizeTimer=setTimeout(renderBrandMarquee,180);
  });
}
