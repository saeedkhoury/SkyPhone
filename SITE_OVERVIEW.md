...
1. **For You (home)** — hero carousel (3 slides, each with a pointer-tracked "3D" tilt plus a continuous idle float animation: an iPhone 18 series "coming soon" full-bleed ad linking to WhatsApp; a real PlayStation 5 ad that links straight into its real, purchasable product page; and a repair-CTA slide) → **Apple Highlights** block (dark, big cards, direct links into each product's PDP) → **Samsung Highlights** block → **Gaming Highlights** block (includes a "GTA VI — coming soon" non-purchasable teaser card linking to WhatsApp) → infinite-scroll brand-wordmark marquee → services grid → repair promo band with live "from ₪X" price chips → gaming banner → **Reels** row (real local `<video>` clips, not embeds) → Instagram follower band (real, verified count).
