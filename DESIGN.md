...
- **Particle field** (`.particle-canvas`, `initParticleField` in `app.js`): replaced the earlier static radial-gradient spotlight on all `.spot` dark surfaces (hero, gaming banner, About badge) with a canvas field of ~25–70 small dash particles. Particles near the cursor ignite in the brand gold with a soft glow (`shadowBlur`) and fade back to dim white when the cursor moves away — the reference was antigravity.google's and gemini.google's cursor-lit particle fields, reworked in Sky Phone's single-accent palette rather than Google's four-color one. Canvas-based (not DOM nodes) for performance; the animation loop only runs while a particle is actually glowing or the pointer is inside the section, and it's gated by the same `canHover` check as the rest of the motion system, so touch and `prefers-reduced-motion` visitors never load it.
