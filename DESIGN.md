...
- **Particle field** (`.particle-canvas`, `initParticleField` in `app.js`): canvas field of ~25–70 small dash particles on the gaming banner and About badge (the smaller `.spot` dark surfaces). Particles near the cursor ignite in the brand gold with a soft glow (`shadowBlur`) and fade back to dim white when the cursor moves away — referenced antigravity.google's and gemini.google's cursor-lit particle fields, reworked in Sky Phone's single-accent palette rather than Google's four-color one. Canvas-based for performance; the animation loop only runs while something is glowing or the pointer is inside the section.
- **Floating badge field** (`.float-badges`, `initFloatingBadges` in `app.js`): hero-only, replaces the particle field there. 16 small glass badges carrying Sky Phone's own catalog/repair icons (phone, repair tools, gaming, etc. — not dev-tool icons), scattered in the hero's side margins so they never sit over the headline/CTA/device. Each badge idle-drifts on independent sine waves, and is pushed away from the cursor with spring physics (stiffness .08, damping .85, 140px influence radius, slight rotational tilt in the push direction) when the pointer is nearby, then glides back to its anchor. Referenced antigravity.google's hero interaction. Hidden below 920px (no room in the mobile single-column layout) and reduced to a static resting layout under `prefers-reduced-motion`.
- Both effects are gated by the same `canHover` check as the rest of the motion system, so touch visitors never load either.
